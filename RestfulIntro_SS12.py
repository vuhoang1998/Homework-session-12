...
class MusicListRes(Resource):
    def get(self): #read
        ...
        #return [json.loads(zodiac.to_json()) for zodiac in Zodiac.objects]
        return meList2json(User.objects)
    def post(self): #Create
        ...
        return{"Status" : "OK"}
        music = User(yourname = yourname,songname= songname,image= image,artists= artists, link = link)
        music.save()


class MusicRes(Resource):
    def get(self,music_id):
        return json.loads(User.objects().with_id(music_id).to_json())
    def delete(self,music_id):
        all_musics = User.objects()
...
print(meList2json(User.objects))
